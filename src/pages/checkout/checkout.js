...
const Checkout = () => {
  ...
  useEffect(() => {
    if (checkout) {
      setTransaction({
        ...
        product_id: checkout[0].product_id,
        total: checkout[0].total + delivery,
  ...
  const handleTransaction = async (e) => {
    ...
    try {
      ...
      await axios.delete(
        `${process.env.REACT_APP_API_BACKEND}checkout/${checkout[0].id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    <div>
      ...
      <div className="container">
        ...
        <div className="d-flex flex-row mt-5">
          <div className={`col-6 m-4 ${styles.box1}`}>
            <h3>Name : {checkout ? checkout[0].fullname : "item is empty"} </h3>
            <p>Address : {checkout ? checkout[0].address : ""}</p>
            <button>Choose your location</button>
          </div>
          <div className={`col-6 p-4 m-4 ${styles.box2}`}>
            ...
            <button className={styles.btn} onClick={handleTransaction}>
              Select Payment
            </button>
          </div>
        </div>
      ...
      <div className={`container`}>
        <h4 className="d-flex mb-4">Product order</h4>
